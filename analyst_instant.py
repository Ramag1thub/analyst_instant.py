# analyst_instant_spot_future_mirror_safe.py
# Final Stable Version — Spot (Yahoo) + Futures (Binance & OKX Mirror)
# Fully Streamlit Cloud Compatible + Error Safe + Chart + Indicators
...
COIN_LIST = [
    ...
    "ENJUSDT","AGIXUSDT","MAGICUSDT","FLRUSDT","ILVUSDT","KSMUSDT","NMRUSDT","RSRUSDT","SFPUSDT",
...
yf_ok = check_yf()
if not yf_ok:
    st.warning("⚠️ Yahoo Finance tidak dapat diakses. Mode Spot mungkin tidak lengkap.")
...
df["conviction"] = df.apply(
    lambda r: (
        "Tinggi"
        if r.get("structure", "No Data") == r.get("fib_bias", "Netral") and r.get("structure", "No Data") in ["Bullish", "Bearish"]
        else "Sedang"
        if r.get("structure", "No Data") in ["Bullish", "Bearish"] or r.get("fib_bias", "Netral") in ["Bullish", "Bearish"]
        else "Rendah"
    ),
    axis=1
)
...
for _, r in bull.iterrows():
    st.markdown(f"**{r['symbol']}** → +{r['change_pct']:.2f}% | {r['structure']}")
...
st.caption("📘 Spot via Yahoo Finance | Futures via Binance & OKX Mirror | Indikator: EMA20/50, Bollinger Bands(20,2), RSI14")
